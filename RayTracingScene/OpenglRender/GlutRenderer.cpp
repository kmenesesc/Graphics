...
void GlutRenderer::ApplyBackgroundColor() {
	glClearColor((GLfloat)BackgroundColor.x, (GLfloat)BackgroundColor.y, (GLfloat)BackgroundColor.z, 1.0f );
}
...
bool GlutRenderer::AddLight( const Light& light )
{
	...
	if ( light.SpotActive() ) {
		...
		glLightf( lightID, GL_SPOT_EXPONENT, (GLfloat)light.GetSpotExponent() );
		glLightf( lightID, GL_SPOT_CUTOFF, (GLfloat)light.GetSpotCutoff() );
	}
	else {
		glLightf( lightID, GL_SPOT_EXPONENT, 0.0 );
		glLightf( lightID, GL_SPOT_CUTOFF, -180.0 );
	}
	glLightf( lightID, GL_CONSTANT_ATTENUATION, (GLfloat)light.GetAttenuateConstant() );
	glLightf( lightID, GL_LINEAR_ATTENUATION, (GLfloat)light.GetAttenuateLinear() );
	glLightf( lightID, GL_QUADRATIC_ATTENUATION, (GLfloat)light.GetAttenuateQuadratic() );
...
void GlutRenderer::RenderViewableTorus( const ViewableTorus& object )
{
	...
	if ( workNm!=0.0 ) {
		...
		glRotated( RadiansToDegrees*theta, work.x, work.y, work.z );
	}
...
void GlutRenderer::SetFaceMaterial( GLenum faceID, const MaterialBase* mat )
{
	...
	glMaterialf( faceID, GL_SHININESS, Min((GLfloat)mat->GetPhongShininess(), 127.00f ) );   // Clamp to < 128.0 for safety.
}
