...
void RayTraceView(void)
{
	...
	float beta;
	const int N = 6;
	const int subPix = N*N;
	VectorR3 averageCol;
	float a;
	float deltaT;
	float iSphere[3] = { 4.0f, 1.2f, 4.0f };
	float fSphere[3] = { 1.0f, 1.2f, 7.0f };
	float nSphere[3] = { 0.0f,0.0f,0.0f };
	int subPix2 = 1;
			...
			for ( j=0; j<WindowHeight; j++ ) {
				...
				if (stoJit==0) {
					...
					for (int k = 0; k < subPix; k++) {
						...
						alpha = ((float)rand() / (float)(RAND_MAX))*((float)1/ (float)N);
						beta = ((float)rand() / (float)(RAND_MAX))*((float)1 / (float)N);
						


						for (int iP = 0; iP < subPix2; iP++) {
							float min = ((float)iP) / (float)subPix2;
							float max = ((float)(iP + 1)) / (float)subPix2;
							a = ((float)(max - min)*((float)rand() / (float)(RAND_MAX))) + min;

							deltaT = a*a;

							for (int jP = 0; jP < 3; jP++) {
								nSphere[jP] = iSphere[jP] * (float)(1 - deltaT) + fSphere[jP] * (float)(deltaT);
							}

							((ViewableSphere*)(ViewObj[29]))->SetCenter(nSphere[0], nSphere[1], nSphere[2]);

							MainView->CalcPixelDirection((float)i + ((float)kx / (float)N), (float)j + ((float)ky / (float)N), &PixelDir); //Shoot the ray
							RayTrace(TraceDepth, MainView->GetPosition(), PixelDir, curPixelColor);



							averageCol += curPixelColor;
						}
					...
					averageCol /= (float)subPix*subPix2;
					pixels->SetPixel(i, j, averageCol);
				}
				else if(stoJit == 1){

					for (int iP = 0; iP < subPix ; iP++) {
						float min = ((float)iP) / (float)subPix;
						float max = ((float)(iP + 1)) / (float)subPix;
						a = ((float)(max - min)*((float)rand() / (float)(RAND_MAX))) + min;
						 ...
						 VectorR3& returnedColor, long avoidK )
{
	...
	for ( int k=0; k<NumLights; k++ ) {
		...
		if ( checksides ) {
			toLight = LightArray[k]->GetPosition();
